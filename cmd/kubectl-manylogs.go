...
Example:
  ...
  kubectl manylogs -namespace="sns-system" -label="control-plane=controller-manager" -container="manager"`,
...
func init() {
	...
	rootCmd.PersistentFlags().StringVar(&cfgFile, "config", "", "config file (default is $HOME/.manylogs.yaml)")
...
func initConfig() {
	if cfgFile != "" {
		...
		// Search config in home directory with name ".manylogs" (without extension).
		viper.AddConfigPath(home)
		viper.SetConfigType("yaml")
		viper.SetConfigName(".manylogs")
	}
